...
export default function AutoGeneratedFieldGroup({
  ...
  useEffect(() => {
    if (preventRefresh) return; // this is used to prevent the component to refresh if state us set while it is nested
    let toUpdate = data;
  ...
  return (
    <>
      {data.map((item, index) => (
        <Grid container key={index} sx={{ minWidth: "30vw" }}>
          {item.type == "text" && (
